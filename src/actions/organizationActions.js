...
export function organizationsLoad() {
  return async function(dispatch, getState) {
    ...
    const organizations = await organizationsApi
      .getOrganizations()
      .then(orgs => orgs)
      .catch(error => {
        ...
        dispatch({ type: types.ORGANIZATIONS_LOAD_ERROR });
      });
    ...
    const allOrganizations = orgDetails.reduce((previous, current) => {
    ...
    const organizationsArray = Object.keys(allOrganizations);
    ...
    return dispatch(
      organizationsLoadSuccess(allOrganizations, selectedOrganization)
    );
  };
}

export function _organizationsLoad() {
