import { Meta, Story, Canvas, ArgsTable } from '@storybook/blocks';
import { Box } from 'grommet';
import Button from '..';

<Meta
  title='Inputs/Button'
  component={Button}
  parameters={{ actions: { argTypesRegex: '^onClick.*' } }}
  argTypes={{}}
/>

# Buttons

## Parameters

### Style

What the styles mean and when to use them:

- `primary` is used when there are multiple buttons, where one is the recommended action or the most important one. Example: a dialog with one conformation button (using the primary style) and one for cancelling (in link style).
- `danger` is used to indicate that the click of the button triggers a destructive action.
- `link` is used in a row with other buttons, in particular for the **Cancel** button in a model dialog.
- `secondary` is for all other cases.

### Size

- The default size works for most contexts.
- The small size (`small`) is used especially in tables and space-constrained contexts.

### Loading indicator

Used to indicate activity, usually as a reaction to the button having been clicked. In this state, the button is also disabled to prevent double triggering.

### Disabling

The button can be disabled independent of other attributes. This shall not be combined with the _loading_ parameter.

## Examples

<ArgsTable of={Button} />

### Multiple buttons

<Canvas>
  <Story name='Multiple'>
    <Box wrap={true} direction='row' gap='small'>
      <Button primary={true}>Primary</Button>
      <Button>Secondary</Button>
      <Button link={true}>Link</Button>
    </Box>
  </Story>
</Canvas>

### Icons and labels

<Canvas>
  <Story name='IconAndLabel'>
    <Box wrap={true} direction='row' gap='small'>
      <Button primary={true} icon={<i className='fa fa-add-circle' />}>Add</Button>
      <Button danger={true} icon={<i className='fa fa-delete' />}>Delete</Button>
    </Box>
  </Story>
</Canvas>

### Secondary

<Canvas>
  <Story name='Secondary'>
    <Button>Secondary</Button>
  </Story>
</Canvas>

<Canvas>
  <Story name='SecondarySmall'>
    <Button size='small'>Secondary small</Button>
  </Story>
</Canvas>

### Primary

<Canvas>
  <Story name='Primary'>
    <Button primary={true}>Primary</Button>
  </Story>
</Canvas>

<Canvas>
  <Story name='PrimarySmall'>
    <Button primary={true} size='small'>Primary small</Button>
  </Story>
</Canvas>

### Danger

<Canvas>
  <Story name='Danger'>
    <Button danger={true}>Danger</Button>
  </Story>
</Canvas>

<Canvas>
  <Story name='DangerSmall'>
    <Button danger={true} size='small'>Danger small</Button>
  </Story>
</Canvas>

### Link

<Canvas>
  <Story name='Link'>
    <Button link={true}>Link</Button>
  </Story>
</Canvas>

<Canvas>
  <Story name='LinkSmall'>
    <Button link={true} size='small'>Link small</Button>
  </Story>
</Canvas>

### Loading

<Canvas>
  <Story name='Loading'>
    <Button loading={true}>Loading</Button>
  </Story>
</Canvas>

<Canvas>
  <Story name='LoadingSmall'>
    <Button loading={true} size='small'>Loading small</Button>
  </Story>
</Canvas>

<Canvas>
  <Story name='LoadingLeft'>
    <Button loading={true} loadignPosition='left'>Loading</Button>
  </Story>
</Canvas>

<Canvas>
  <Story name='LoadingLeftSmall'>
    <Button loading={true} loadignPosition='left' size='small'>Loading small</Button>
  </Story>
</Canvas>

### Disabled

<Canvas>
  <Story name='Disabled'>
    <Button disabled={true}>Disabled</Button>
  </Story>
</Canvas>

<Canvas>
  <Story name='DisabledSmall'>
    <Button disabled={true} size='small'>Disabled small</Button>
  </Story>
</Canvas>

### Unauthorized

<Canvas>
  <Story name='Unauthorized'>
    <Button disabled={true} unauthorized={true}>Unauthorized</Button>
  </Story>
</Canvas>

<Canvas>
  <Story name='UnauthorizedSmall'>
    <Button disabled={true} unauthorized={true} size='small'>Unauthorized small</Button>
  </Story>
</Canvas>
