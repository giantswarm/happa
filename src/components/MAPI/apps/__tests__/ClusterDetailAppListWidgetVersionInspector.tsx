...
describe('ClusterDetailAppListWidgetVersionInspector', () => {
  ...
  it('displays various information about the supported app versions', async () => {
    ...
    expect(await screen.findByText(currentVersion)).toBeInTheDocument();
    expect(
      screen.getByText('released less than a minute ago')
    ).toBeInTheDocument();
    expect(
  ...
  it('displays current version even if it is not in the list of supported versions', async () => {
    ...
    expect(await screen.findByText(currentVersion)).toBeInTheDocument();
    expect(screen.getAllByLabelText('no information available')).toHaveLength(
      2
    );
    expect(screen.getByText('(current version)')).toBeInTheDocument();
  });
