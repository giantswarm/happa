...
Under the `scope` field we define whether the permissions for this use case are defined in the cluster scope, or in a paerticular namespace, or in any namespace.
...
In contrast, if a use case's permissions are to be defined in namespaces, `scope.cluster` can be omitted ot set to `false`. In addition, the `namespaces` have to be enumerated, or an `*` string must be used to indicate that the permissions can be looked up in any namespace.
